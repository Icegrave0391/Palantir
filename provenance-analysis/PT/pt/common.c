...
circular_addr_t *circular_addr_init() {
    circular_addr_t* addr_c = (circular_addr_t *) malloc(sizeof(circular_addr_t));

    addr_c->head = -1;
    addr_c->max = CIRCULAR_SIZE;  
    addr_c->previous_addr = (unsigned long*) malloc(sizeof(unsigned long) * CIRCULAR_SIZE);
    memset((void *) addr_c->previous_addr, 0, sizeof(unsigned long) * CIRCULAR_SIZE);

    return addr_c;
}

void circular_addr_free(circular_addr_t * addr_c) {
    free(addr_c->previous_addr);
    free(addr_c);
}

void circular_addr_put(circular_addr_t * addr_c, unsigned long addr) {
    if (addr_c->head == (addr_c->max - 1)) {
        addr_c->head = 0;
    }
    else {
        addr_c->head += 1;
    ...
    addr_c->previous_addr[addr_c->head] = addr;
}

unsigned long circular_addr_get(circular_addr_t * addr_c, int back) {
    int curr;
    if ((addr_c->head - back) < 0) {
        curr = addr_c->head - back + addr_c->max;
    }
    else {
        curr = addr_c->head - back;
    }

    unsigned long addr = addr_c->previous_addr[curr];
