...
bool KG::InsertLoginProc(const Json::Value event, LogLoader *logload, event_t id) {
	...
	if (p) {
		...
		for (event_t id_tmp = id; id_tmp < (id + 10); id_tmp++) {
...
// print edges related to a process
std::vector<KGEdge *> KG::PrintEdges(std::string exe, std::string pid) {
	// search a process by exe/pid in ProcNodeTable
	std::vector<hash_t> p_hash_vec;
	for (auto node : ProcNodeTable){
		if (node.second->exe == exe) {
			if (pid == "-1") {
				p_hash_vec.push_back(*(node.second->id));
			}
			else if (node.second->pid == pid) {
				p_hash_vec.push_back(*(node.second->id));
				break;
			}
		}
	}
	if (p_hash_vec.size() == 0) {
		std::cerr << "Cannot find " << exe << "in ProcNodeTable" << std::endl;
		exit(-1);
	}
	
	// collect and sort edges related to the process
	std::vector<KGEdge *> edges_of_interest;

	for (auto p_hash: p_hash_vec) {
		std::map<seq_t, KGEdge *> proc_edges;

		for (auto it: KGEdgeTable){
			KGEdge *edge = it.second;

			// Todo: add more system calls for intel PT analysis
			if (edge->relation != EdgeType_t::Recv and 
				edge->relation != EdgeType_t::Read and
				edge->relation != EdgeType_t::Pread and 
				edge->relation != EdgeType_t::Write and
				edge->relation != EdgeType_t::Writev)
					continue;

			if (edge->n1_hash == p_hash or edge->n2_hash == p_hash) {
				proc_edges[edge->seq] = edge;
				edges_of_interest.push_back(edge);
			}
		}

		std::cout << "Edges related to: " << SearchNode(p_hash).second << std::endl;
		int syscall_seq = 0;
		for (auto it: proc_edges) {
			KGEdge *edge = it.second;
			std::string n_info;
			hash_t n_hash;
			if (edge->n1_hash == p_hash) {
				n_info = SearchNode(edge->n2_hash).second;
				n_hash = edge->n2_hash;
			}
			else {
				n_info = SearchNode(edge->n1_hash).second;
				n_hash = edge->n1_hash;
			}

			std::cout << syscall_seq << " " << uint128tostring(edge->seq) << ": " << EdgeEnum2String(edge->relation);
			std::cout << "\t" << n_info << "\t" << n_hash << std::endl;
			syscall_seq += 1;
		}
		std::cout << std::endl;
	}
