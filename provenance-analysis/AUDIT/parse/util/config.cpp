...
void Config::ConfigBeat() {
	...
	// define a process by exe to lookop related edges
	if(ComOptionExist("-lookupproc")) {
		lookupproc = true;
		lookup_proc_exe = GetComOption("-lookupproc");
		lookup_proc_pid = GetSecComOption("-lookupproc");
		try {
			std::stoi(lookup_proc_pid);
		} catch (...) {
			lookup_proc_pid = "-1";
		}
	}
