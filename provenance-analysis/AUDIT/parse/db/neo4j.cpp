...
void Neo4jdb::Neo4jVizEdge(std::vector<KGEdge*> edges) {
	std::cout << "Start to Visualize Edges" << std::endl;
	...
	for (const auto &edge: edges) {
		...
		std::string pt_str = edge->pt_str;
		std::string timestamp = edge->timestamp;
		std::string e_cmd = "CREATE(n1)-[r:"+rel_str+"{seq:"+seq_str+", timestamp:\""+timestamp+"\", pt:\"" + pt_str + "\"}]->(n2)";
		cmd = n1_cmd + n2_cmd + e_cmd;

		run(cmd);
	}

	std::cout << "Finish Visualizing Edges" << std::endl;
}
...
std::string Neo4jdb::EdgeCmd(KGEdge *edge, event_t e_seq) {
	...
	// create edge
	std::string e_cmd = "CREATE("+n1_str+")-["+r_str+":"+rel_str+
		"{seq:" + seq_str + 
		", timestamp:\"" + timestamp +
		"\", pt:\"" + pt_str + "\"}]->("+n2_str+") with "+r_str+" ";
