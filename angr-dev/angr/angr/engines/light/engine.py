...
class SimEngineLight(
    ...
    def _codeloc(self, block_only=False):
        """
        In this fs_engine, the granularity of instruction is ins_addr, rather than IR stmt id,
        thus it's ok to pass the stmt_id
        """
        return CodeLocation(self.block.addr, None, ins_addr=self.ins_addr, context=self._context)
